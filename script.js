...
fetch(masjidUsSunnahApi)
  ...
  .then((data) => {
    ...
    const calanderArray = firstMonthObjects.map((date) => {
      ...
      return `<section class="flex items-center ${
        dayName === "Monday" ? "bg-[#A4D7F4]" : ""
      } ${
        dayName === "Friday" ? "font-semibold" : ""
                ...
                <div class="border-[#26A5D2] border-r py-1">
                  <input
                    name="Jumu'Asr"
                    type="text"
                    value="${date.asr_mithl_2.substring(0, 5)}"
                    class="px-1 text-center ${
                      dayName === "Monday" ? "bg-[#A4D7F4]" : ""
                    } outline-none w-[58px]"
                  />
...
// let url = `https://api.aladhan.com/v1/calendar/${yearPrayerTimings[0].d_date.substring(
//   0,
//   4
// )}?latitude=53.83333000&longitude=-1.83333000&method=2`;
...
//   })
//   .then((data) => {
//     // Store the result in a variable (use 'data' as needed)
//     let apiResult = data;
//     console.log("Second Api:", apiResult);
...
function handleInputChange(inputId) {
  ...
  if (inputElement) {
    ...
    // You can use the variables (solarMonthValue, solarYearValue, islamicMonthValue) as needed
